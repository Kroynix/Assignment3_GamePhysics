...
Button2::Button2(const std::string& image_path, std::string button_name, const GameObjectType type,
	const glm::vec2 position, const bool is_centered) :
	m_alpha(255), m_name(std::move(button_name)), m_isCentered(is_centered), m_active(true)
{
	TextureManager::Instance()->load(image_path, m_name);
 ...
 * @return void
 */
void Button2::setAlpha(const Uint8 alpha)
{
	m_alpha = alpha;
}
 ...
 * @return void
 */
void Button2::setActive(const bool value)
{
	m_active = value;
}
